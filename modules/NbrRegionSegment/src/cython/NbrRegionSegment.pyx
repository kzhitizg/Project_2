...
cdef extern from "../cpp/NbrRegionSegment.h":
    cdef void Segment(unsigned char *n, unsigned char *l, int x, int y, int z, int thres, float wr, float wg, float wb, float wc, unsigned char *Y)
    cdef int GetRegions(unsigned char *n, unsigned char *l, int x, int y, int z, int thres, float wr, float wg, float wb, float wc, int *ret, int *count)
    cdef void RemoveMap(int * n, int x, int y, int thres, int *count, int regs, bool *ret)
    cdef void SegmentAndRemove(unsigned char * n, unsigned char *l, int x, int y, int z, int thres1, int thres2, float wr, float wg, float wb, float wc, unsigned char *ret)
...
def _SegmentWrapper(np.ndarray[unsigned char, ndim=3] X, np.ndarray[unsigned char, ndim=2] lbp, thres, wr, wg, wb, wc):
    ...
    # Call the function
    Segment(&X[0,0, 0], &lbp[0, 0], X.shape[0], X.shape[1], X.shape[2], thres, wr, wg, wb, wc, &Y[0, 0, 0])

    # Return array
    return Y

def _RegionExtractWrapper(np.ndarray[unsigned char, ndim=3] X, np.ndarray[unsigned char, ndim=2] lbp, thres, wr, wg, wb, wc):
    # Make the array allocation continuous
    X = np.ascontiguousarray(X)
    lbp = np.ascontiguousarray(lbp)
    ...
    # Call the function
    reg = GetRegions(&X[0,0, 0], &lbp[0, 0], X.shape[0], X.shape[1], X.shape[2], thres, wr, wg, wb, wc, &Y[0, 0], &regions[0])

    # Return array
    return Y, regions[:reg]

def _RemoveBG(np.ndarray[unsigned char, ndim=3] X,  np.ndarray[unsigned char, ndim=2] lbp, thres1, thres2, wr, wg, wb, wc):
    # Make the array allocation continuous
    X = np.ascontiguousarray(X)
    lbp = np.ascontiguousarray(lbp)
    ...
    # Call the function
    SegmentAndRemove(&X[0,0, 0], &lbp[0, 0], X.shape[0], X.shape[1], X.shape[2], thres1, thres2, wr, wg, wb, wc, &Y[0, 0, 0])
